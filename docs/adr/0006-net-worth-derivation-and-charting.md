...
12. **`sr-only` belongs on a wrapper around the data table, never on the table.** A `<caption>` box is generated by the table _wrapper_ box and sits outside the table grid, so the `overflow: hidden` and 1px sizing that `sr-only` applies to the table never clip it. The caption rendered at full size, on the page, over the chart. Hiding a block-level ancestor takes the caption with it. Guarded by a test, because nothing about the markup looks wrong.

13. **A sparkline's text alternative is a sentence, not a data table** — narrowing decision 8, which stands unchanged for the full-size chart. The Chase flow view's context sparkline (issue [#25](https://github.com/bsim0927/ben-os/issues/25)) shares `areaChartGeometry` with `AreaChart`, per decision 7 and the consequence below, but not its hidden table. A sparkline's job is shape; its endpoint is already printed beside it as the panel's `Net` figure, so a table would make a screen reader walk thirty dated rows to rediscover a number it had just been given, and would repeat that per account. It carries `sr-only` prose instead — what the line is of, and where it ends.

    Decision 8's second reason, that a text alternative is what makes a chart _assertable_, is met the same way: the sentence names the figure, so the page test still reads the sparkline's meaning off the DOM rather than off a path.
