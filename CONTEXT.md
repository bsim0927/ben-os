...
**Current holdings**:
The Holdings stamped with an Account's _latest_ `as_of` — the whole reading, taken together. Deliberately not "the newest row per Account/Security pair", which sounds equivalent and is not: a position sold between two syncs has no later row to supersede it, so that reading would carry it as a holding forever. Resolved per Account, because the two Fidelity accounts sync seconds apart and a single maximum across both would empty whichever finished first. See [ADR 0009](docs/adr/0009-fidelity-holdings-page.md).
